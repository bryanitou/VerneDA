...
private:
    ...
    // Unique ptr
    std::unique_ptr<mex_aux> aux = std::make_unique<mex_aux>();

    // Get pointer to engine
    matlabSharedPtr matlabPtr = getEngine();

    // Get array factory
    matlabFactory matlabFact;
...
public:
    void operator()(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs) override {
        try {
            ...
            // Extract inputs
            auto ini_state = this->aux->convertMatlabTypedArray2NormalVector(inputs[0]);
            auto stddev = this->aux->convertMatlabTypedArray2NormalVector(inputs[1]);
            auto t = this->aux->convertMatlabTypedArray2NormalVector(inputs[2]);
            auto ci = this->aux->convertMatlabDouble2NormalDouble(inputs[3]);
            auto nli = this->aux->convertMatlabDouble2NormalDouble(inputs[4]);
            auto n_max = this->aux->convertMatlabInt2NormalInt(inputs[5]);
            auto n_samples = this->aux->convertMatlabInt2NormalInt(inputs[6]);
            auto str_alg = this->aux->convertMatlabStr2NormalStr(inputs[7]);
            auto str_enum =
            ...
            if (str_enum == PROBLEM::FREE_TORQUE_MOTION) {
                inertia = this->aux->convertMatlab3x3Array2Normal3x3Array(inputs[8], this->matlabPtr, &this->matlabFact);
            }
            ...
            if (n_max == - 1) {
                ...
                this->matlabPtr->feval(u"fprintf",
                                 ...
                                 std::vector<matlab::data::Array>(
                                         {this->matlabFact.createScalar("DA Simple propagation selected...\n")}));
                ...
                outputs[0] = this->aux->convertNormalStr2MatlabStr(fin_state, &this->matlabFact);
            } else // LOADS propagation
            {
                ...
                // Convert all states to matlab array
                outputs[0] = this->aux->convertNormalVector2MatlabTypedArray(*fin_state, &this->matlabFact);
            }
    ...
    void checkArguments(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs) {
        ...
        for (auto &i: vectors2check6) {
            ...
            if (print_error) {
                ...
                this->matlabPtr->feval(u"error",
                                 0,
                                 std::vector<matlab::data::Array>(
                                         {this->matlabFact.createScalar(err2print)}));
            }
        }
        ...
        if (outputs.size() > 1) {
            this->matlabPtr->feval(u"error",
                             0,
                             std::vector<matlab::data::Array>({this->matlabFact.createScalar("Only one output is returned")}));
        }
    }
    ...
    propagate_orbit_loads_SUPER_MANIFOLD(const std::vector<double> &t, double &nli, int &n_max, PROBLEM prob,
        ...
        this->matlabPtr->feval(u"fprintf",
                         0,
                         std::vector<matlab::data::Array>({this->matlabFact.createScalar(scv0.toString())}));
